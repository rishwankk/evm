...
const POLL_DATA = {
  ward: {
    label: "Ward",
    candidates: [
      { id: 1, name: "ഫിറോസ് ഖാൻ കരിമ്പിൽ ", symbol: ladderSymbol, sub: "POOLAMANNA - WARD 12" },
  ...
  block: {
    ...
    candidates: [
      ...
      { id: 2, name: "സഫീർ ജാൻ ", symbol: handsymbol,sub:"വണ്ടൂർ ബ്ലോക്ക് പഞ്ചായത്ത് പാണ്ടിക്കാട് ഡിവിഷൻ " },
      // ... fill up to 10
...
const BallotUnit = () => {
  ...
  return (
    <div className="container">
      {/* Tabs */}
      <div className="tabs">
        <button className={`tab-pill tab-grey ${activeTab === 'ward' ? 'active' : ''}`} onClick={() => handleTabChange('ward')}>Ward</button>
        <button className={`tab-pill tab-pink ${activeTab === 'block' ? 'active' : ''}`} onClick={() => handleTabChange('block')}>Block</button>
        <button className={`tab-pill tab-blue ${activeTab === 'district' ? 'active' : ''}`} onClick={() => handleTabChange('district')}>District</button>
      </div>

      {/* EVM */}
      <div className="ballot-box">
        <div className="status-bar">
          <div>Ready <span className={`ready-light ${isReady ? 'on' : 'off'}`}></span></div>
          ...
          {currentData.candidates.map((c) => (
            <div key={c.id} className="candidate-row">
              ...
              <div className="candidate-info">
                ...
                {c.symbol && <img src={c.symbol} alt="s" style={{height:'40px'}} />}
              </div>
              <div className="control-panel">
                <div className={`led ${activeLed === c.id ? 'on' : ''}`}></div>
                <button className="vote-btn" onClick={() => handleVote(c)} disabled={isVoting}></button>
              </div>
            </div>
          ))}
        </div>
